...
from uniqa.tools.errors import SchemaGenerationError
from uniqa.tools.from_function import _remove_title_from_schema
from uniqa.tools.parameters_schema_utils import _get_component_param_descriptions, _resolve_type
# from uniqa.utils.callable_serialization import deserialize_callable, serialize_callable
...
class ComponentTool(Tool):
    """
    A Tool that wraps uniqa components, allowing them to be used as tools by LLMs.
    ...
    To use ComponentTool, you first need a uniqa component - either an existing one or a new one you create.
    ...
    ```python
    from uniqa import component, Pipeline
    from uniqa.tools import ComponentTool
    from uniqa.components.websearch import SerperDevWebSearch
    from uniqa.utils import Secret
    from uniqa.components.tools.tool_invoker import ToolInvoker
    from uniqa.components.generators.chat import OpenAIChatGenerator
    ...
    def __init__(
        ...
        *,
        ...
        """
        Create a Tool instance from a uniqa component.

        :param component: The uniqa component to wrap as a tool.
        ...
        if not isinstance(component, Component):
            message = (
                f"Object {component!r} is not a uniqa component. "
                "Use ComponentTool only with uniqa component instances."
            )
            raise ValueError(message)

        if getattr(component, "__uniqa_added_to_pipeline__", None):
            msg = (
        ...
        def component_invoker(**kwargs):
            ...
            """
            converted_kwargs = {}
            input_sockets = component.__uniqa_input__._sockets_dict  # type: ignore[attr-defined]
    ...
    def _create_tool_parameters_schema(self, component: Component, inputs_from_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for input_name, socket in component.__uniqa_input__._sockets_dict.items():  # type: ignore[attr-defined]
