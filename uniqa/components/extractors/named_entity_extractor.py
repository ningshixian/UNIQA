...
class NamedEntityExtractor:
    ...
    该组件支持两种后端：Hugging Face 和 spaCy。
    前者可与[Hugging Face model hub](https://huggingface.co/models)中的任何序列分类模型配合使用，
    而后者可与任何包含命名实体识别（NER）组件的[spaCy model](https://spacy.io/models)配合使用。
    标注实体信息作为「元数据」存储在文档中。
