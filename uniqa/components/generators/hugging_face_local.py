...
with LazyImport(message="Run 'pip install \"transformers[torch]\"'") as transformers_import:
    ...
    from uniqa.utils.hf import (  # pylint: disable=ungrouped-imports
...
class HuggingFaceLocalGenerator:
    ...
    def warm_up(self):
        ...
        if self.pipeline is None:
            # import torch
            # self.huggingface_pipeline_kwargs["torch_dtype"] = torch.float16   # RuntimeError: isin_Tensor_Tensor_out only works on floating types on MPS for pre MacOS_14_0. Received dtype: Long
            # self.huggingface_pipeline_kwargs["torch_dtype"] = "auto"  # TypeError: MPS BFloat16 is only supported on MacOS 14 or newer
