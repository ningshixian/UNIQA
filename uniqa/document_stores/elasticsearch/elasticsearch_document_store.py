...
with LazyImport(message="Run 'pip install elastic_transport'") as es_import:
    from elastic_transport import NodeConfig
es_import.check()

Hosts = Union[str, List[Union[str, Mapping[str, Union[str, int]], NodeConfig]]]
uniqa_version = "2.16.0-rc0"
...
class ElasticsearchDocumentStore:
    ...
    ```python
    from uniqa.document_store.elasticsearch import ElasticsearchDocumentStore
    ...
    def _ensure_initialized(self):
        ...
        if not self._initialized:
            ...
            headers["user-agent"] = f"uniqa-py-ds/{uniqa_version}"
    ...
    def filter_documents(self, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        if filters and "operator" not in filters and "conditions" not in filters:
            msg = "Invalid filter syntax. See https://docs.uniqa.deepset.ai/docs/metadata-filtering for details."
            raise ValueError(msg)
    ...
    def write_documents(self, documents: List[Document], policy: DuplicatePolicy = DuplicatePolicy.NONE) -> int:
        ...
        :raises Exception: If a document with the same ID already exists in the document store and
            `policy` is set to `DuplicatePolicy.FAIL` or `DuplicatePolicy.NONE`.
        :raises Exception: If an error occurs while writing the documents to the document store.
