...
from uniqa.dataclasses.byte_stream import ByteStream
from uniqa.dataclasses.sparse_embedding import SparseEmbedding

"""pytest -svx test/dataclasses/test_byte_stream.py"""
...
def test_from_dict_with_dataframe():
    ...
    document.to_dict(flatten=False) in uniqa-ai<=2.10.0.
    We make sure that Document.from_dict() does not raise an error and that dataframe is skipped (legacy field).
    """

    # Document dictionary obtained with document.to_dict(flatten=False) in uniqa-ai<=2.10.0
    doc_dict = {
        "id": "my_id",
        "content": "my_content",
        "dataframe": None,
