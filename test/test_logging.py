...
import uniqa.utils.jupyter
from uniqa import logging as uniqa_logging
...
class TestSkipLoggingConfiguration:
    def test_skip_logging_configuration(
        ...
        monkeypatch.setenv("uniqa_LOGGING_IGNORE_STRUCTLOG", "true")
        uniqa_logging.configure_logging()
...
class TestStructuredLoggingConsoleRendering:
    def test_log_filtering_when_using_debug(self, capfd: CaptureFixture) -> None:
        uniqa_logging.configure_logging(use_json=False)
    ...
    def test_console_rendered_structured_log_even_in_jupyter(
        ...
        monkeypatch.setattr(uniqa.utils.jupyter, uniqa.utils.jupyter.is_in_jupyter.__name__, lambda: True)
    ...
    def test_console_rendered_structured_log_even_if_no_tty_but_forced_through_env(
        ...
        monkeypatch.setenv("uniqa_LOGGING_USE_JSON", "false")
...
class TestStructuredLoggingJSONRendering:
    ...
    def test_logging_as_json(self, capfd: CaptureFixture) -> None:
        uniqa_logging.configure_logging(use_json=True)
    ...
    def test_logging_as_json_enabling_via_env(self, capfd: CaptureFixture, monkeypatch: MonkeyPatch) -> None:
        monkeypatch.setenv("uniqa_LOGGING_USE_JSON", "true")
        uniqa_logging.configure_logging()
...
class TestCompositeLogger:
    ...
    def test_correct_stack_level_with_consoler_rendering(self, capfd: CaptureFixture) -> None:
        ...
        logger = uniqa_logging.getLogger(__name__)
        logger.warning("Hello, structured logging!", extra={"key1": "value1", "key2": "value2"})
    ...
    def test_uniqa_logger_with_positional_args(self, method: str, expected_level: str) -> None:
        uniqa_logging.configure_logging(use_json=True)
    ...
    def test_uniqa_logger_with_old_interpolation(self, method: str, expected_level: str) -> None:
        uniqa_logging.configure_logging(use_json=True)
    ...
    def test_that_uniqa_logger_is_used(self) -> None:
        """Forces the usage of the uniqa logger instead of the standard library logger."""
        allowed_list = [Path("uniqa") / "logging.py"]
        for root, dirs, files in os.walk("uniqa"):
            for file in files:
                path = Path(root) / file
                ...
                if " logging.getLogger" in content:
                    uniqa_logger_in_content = " uniqa import logging" in content or ", logging" in content
                    assert uniqa_logger_in_content, (
                        f"{path} doesn't use the uniqa logger. Please use the uniqa logger instead of the "
